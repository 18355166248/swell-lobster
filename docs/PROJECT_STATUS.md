...
- 后端：Node.js 20.20.0 / Hono 4 / SQLite (better-sqlite3)
- 前端：React 19 / TypeScript / Vite 8 / Ant Design 6

---

## ✅ 已完成功能（阶段 1-14 + 阶段 15a/15b/15c + 阶段 16 已完成）
...
- 统一观测事件模型（`observability/eventTypes.ts`）：12 种 EventCategory，chat/tool/plan/delegate/im/scheduler/mcp 全链路覆盖
- trace 持久化（`observability/traceStore.ts`）：事件写入、失败查询、慢调用查询、30天自动清理
- 指标聚合（`observability/metrics.ts`）：按分类成功率、平均/P95 耗时、每日趋势、24h/7d 摘要
- 备份与恢复（`db/backup.ts`）：SQLite + `data/` 关键目录备份，pre-restore 回滚保护；API `POST/GET /api/backup/*`
- DB 迁移收敛：migration 可重复执行、失败不破坏已完成版本、启动时打印当前版本
- 统一错误码（`types/errors.ts`）：`AppError` 类 + 5 种 `ErrorCode`，API 错误统一 `{ detail, code? }` 格式
- Status 页观测面板（+302行）：最近失败调用、慢调用、审批阻塞、MCP 健康状态
- 发布前稳定性基线：`scripts/verify.mjs` 补充 observability / backup smoke test，`desktop-validation-checklist.md` 全部通过

### 阶段15a：安全底座 ✅

- API 鉴权与远程 token 管理已落地（本机 token + `auth_tokens`）
- 鉴权失败限流与 CORS 白名单已落地
- 关键 POST/PATCH 路由已接入 zod 输入校验
- 敏感字段 AES-256-GCM 加密、主密钥管理与迁移已落地
- 前端 Security 设置页已可查看主密钥状态、管理远程 token、控制远程访问开关

### 阶段15b：文档导出能力 ✅

- 已落地 `docx_writer` / `xlsx_writer` / `pptx_writer` 3 个内置工具，并注册到 builtin tool registry
- 已补齐 Word / Excel / PPT 生成回归测试，覆盖输出目录写入与核心 schema
- 已新增 3 个文档技能模板：`docx_report`、`xlsx_table`、`pptx_brief`
- 前端 Skills 页已支持 `document` 分类，可按“文档生成”分组查看与筛选
...
阶段 15a / 15b / 15c 与阶段 16a / 16b / 16c 的核心交付已全部落地，根级 `npm run verify` 通过（40 文件 / 199 用例）。下一步可进入阶段 17 规划或补充部署文档。

### 阶段 15 当前范围

**阶段 15a：安全底座（已完成）**

- [x] API 鉴权（本机 token 自动注入 + 远程 token 管理）
- [x] 字段级 AES-256-GCM 加密（IM Token / LLM API Key / SMTP 密码）
- [x] 输入校验（zod schema 化关键 POST/PATCH）
- [x] CORS 白名单收紧（默认仅放行桌面端与本地开发源）

**阶段 15b：文档导出能力（已完成）**

- [x] `docx_writer`（基于 docx）
- [x] `xlsx_writer`（基于 exceljs，多 sheet + 表头冻结）
- [x] `pptx_writer`（基于 pptxgenjs，4 种 layout）
- [x] 3 个对应的 markdown 技能模板
- [x] 前端 Skills 页“文档生成”分组

**阶段 15c：自动化能力（已完成）**

- [x] `browser_automation`（基于 playwright，screenshot / extract_text / fill_and_submit）
- [x] `email_send`（基于 nodemailer，SMTP MVP）
- [x] SMTP 配置落 `key_value_store.email.smtp.config`
- [x] `browser_capture` / `send_email` 技能模板与 Skills 页「自动化」分组

### 阶段 16 ✅（2026-05-15 完成）

**16a：统一出站网络策略（已完成）**

- [x] 新建 `net/outboundPolicy.ts`，`open / allowlist` 两种模式
- [x] `browser_automation`、`web_search`、`email_send` 接入 `checkOutbound()`
- [x] `store/sandboxConfig.ts` + `api/routes/configSandbox.ts`（GET/PATCH/POST/DELETE）
- [x] 补回归测试

**16b：run_script 子进程环境净化（已完成）**

- [x] 新建 `utils/sanitizeEnv.ts`，剥离 `SWELL_*`、`*_API_KEY`、`*_TOKEN` 等敏感变量
- [x] `run_script.ts` 接入 `sanitizeChildEnv()`
- [x] 补回归测试

**16c：前端沙箱与网络设置页（已完成）**

- [x] Security 页新增 `SandboxPanel.tsx`（出站模式切换 + 白名单管理）
- [x] 对接 `/api/config/sandbox`

### 阶段 16 已全部完成，验收通过

根级 `npm run verify` 通过（40 文件 / 199 用例）。
...
SwellLobster 项目已完成全部 14 个阶段，具备一个完整个人 AI 助手所需的全部核心功能：
...
✅ 观测性与稳定性（统一事件模型、trace、指标聚合、备份恢复、统一错误码）

阶段 1-14 全部完成，桌面端实机验证通过，系统已达到生产就绪标准。
