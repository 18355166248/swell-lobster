...
| 阶段 13（步骤1-5 后端）：统一扩展模型 + catalog + manifest + API     | ✅   | ⬜   |

### 核心功能完整度

当前已完成阶段 1-7、阶段 9、阶段 10、阶段 11、阶段 12，阶段 13 已完成后端基线，系统具备：
...
- 统一扩展后端目录（builtin / skill / mcp catalog、manifest、健康状态与生命周期 API）
...
阶段13：统一扩展运行时 🟡    → 后端 catalog、manifest、健康状态与 API 已完成，前端入口与审计联动待补
...
3. 阶段 13 前端入口：补统一扩展列表页，接入来源、健康状态、权限画像与启停动作。
4. 阶段 13 审计联动：在执行审计中补充扩展来源字段，并支持按来源过滤。
5. 阶段 14：补观测性、迁移、备份恢复与发布基线，达到生产就绪标准。
