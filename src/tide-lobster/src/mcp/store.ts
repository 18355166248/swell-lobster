...
import type { MCPServerConfig, MCPServerCreateInput, MCPServerStatus } from './types.js';
...
export class MCPStore {
    ...
    this.db
      .prepare(
        `INSERT INTO mcp_servers (
          id, name, command, args, env, enabled, status, error_message, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, 'stopped', NULL, ?, ?)`
      )
      .run(
        id,
  ...
  update(
    id: string,
    patch: Partial<Pick<MCPServerConfig, 'name' | 'command' | 'args' | 'env' | 'enabled'>>
  ): MCPServerConfig {
