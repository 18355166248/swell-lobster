...
阶段 15a / 15b 的核心交付已在代码中落地。下一步主线切换到 **阶段 15c：自动化能力**，并在启动 15c 前补一次根级验收与状态文档收口，方案见 [phases/phase15-security-productivity-skills.md](./phases/phase15-security-productivity-skills.md)。
...
- [ ] SMTP 配置落 `key_value_store.email.smtp.config`

### 建议执行顺序

1. 先执行根级 `npm run verify`，补一次阶段 15b 的统一验收。
2. 启动 `browser_automation`，优先打通域名白名单、审批门和截图落盘链路。
3. 再落 `email_send`，复用 15a 已完成的密文字段存储与审批能力。
