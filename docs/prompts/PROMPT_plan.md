...
用于: 分析需求 → 生成/更新 identity/MEMORY.md 中的计划
-->
...
0c. 学习 `src/swell-lobster/` 了解现有代码结构和模式。
...
- 仅做计划，不要实现任何东西
- 不要假设功能缺失，先用代码搜索确认
- 将 `src/swell-lobster/skills/` 视为技能库
- 优先使用已有的工具和模式

**最终目标**:
我们要实现一个全能自进化AI Agent。考虑缺失的元素并相应规划。如果元素缺失，先搜索确认不存在，然后在 `specs/` 创建规格文档，在 `identity/MEMORY.md` 中记录实现计划。
