...
- Embedding 服务与配置项已落地
- `read_memory` 已支持语义检索优先，失败时降级到关键词检索
- `web_search` 已支持 Brave / Tavily / DuckDuckGo 与 fallback
- 高级配置页已提供 Embedding 与搜索 API Key 配置
- 尚缺迁移脚本、提供商优先级显式配置、完整回归测试与文档收口
...
- 固化仓库级验证入口：`npm run verify`
- 已补主干回归测试：
  - Journal 记忆联动 API
  - Agent 模板创建会话链路
  - `read_memory` / `web_search` 回归测试
- 下一步优先补桌面 sidecar 与导出链路回归

### 优先级2：阶段9 向量记忆 + 网络搜索

- 增加 embedding 回填 / 迁移脚本
- 补齐搜索提供商优先级显式配置
- 明确语义检索阈值与无结果回退策略
