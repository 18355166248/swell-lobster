const zh = {
  ...
  security: {
    title: '安全设置',
    subtitle: '管理本机令牌、远程访问、SMTP 发信配置与主密钥状态。',
    actions: {
      revoke: '撤销',
    },
    masterKey: {
      ...
      alertTitle: '主密钥缺失',
      alertDescription:
        '所有受保护字段（IM Token、Webhook Secret、SMTP 密码等）将以 null 返回。请检查 data/auth/master.key 是否被删除，或恢复备份。',
    },
    localToken: {
      title: '本机 token',
      description:
        '本机 token 由 sidecar 启动时自动生成，桌面端 UI 会自动注入到请求。重置后旧 token 会立即失效。',
      reset: '重置本机 token',
      resetConfirm: '重置后所有现有连接都需要重新获取 token，确认？',
    },
    remote: {
      title: '远程访问',
      description:
      ...
      riskTitle: '风险提示',
      riskDescription:
        '启用后任何能访问本机端口的设备都将能调用 LLM 凭据 / 邮箱 / IM 通道。仅在受信网络下开启。',
      disableConfirmTitle: '是否同时撤销所有远程 token？',
      keepTokens: '保留 token，仅关闭开关',
    ...
    smtp: {
      title: 'SMTP 发信配置',
      description:
        '配置后，`email_send` 工具和自动化邮件技能可通过 SMTP 发信。密码会以字段级加密方式存储。',
      host: 'SMTP 主机',
      port: '端口',
      user: '用户名',
      from: '发件地址',
      ...
      portRequired: '请输入端口',
      userRequired: '请输入用户名',
      fromRequired: '请输入发件地址',
      passwordPlaceholder: '留空表示沿用当前已保存密码',
      passwordRetainHint: '留空可沿用当前已保存密码；输入新值会覆盖旧密码。',
      passwordRequiredHint: '首次保存时必须填写密码。',
      savedTitle: '当前已保存 SMTP 配置',
      savedDescription: '当前主机：{{host}}，默认发件人：{{from}}。',
    },
    remoteTokens: {
      title: '远程访问令牌',
      create: '新建 token',
      label: '标签',
      labelRequired: '请输入标签（如 mac / mobile / hometown）',
    },
    tokenTable: {
      id: 'ID',
      label: '标签',
      ...
      createdAt: '创建时间',
      lastUsedAt: '最近使用',
      revokeConfirm: '撤销后该 token 会立即不可用，确认？',
    },
    createdToken: {
      title: '新 token 已创建（仅本次显示）',
      copy: '复制 token',
      saved: '我已保存',
      warning: '请立即保存此 token，关闭后将无法再次查看。',
      label: '标签：',
      scope: '权限：',
    },
    messages: {
      revoked: '已撤销',
      localTokenReset: '本机 token 已重置',
      remoteEnabled: '已启用远程访问；重启服务后自动以 0.0.0.0:18900 监听。',
      remoteDisabled: '已关闭远程访问',
      remoteDisabledRevoked: '已关闭远程访问，并撤销了 {{count}} 个 token',
      copied: '已复制到剪贴板',
    ...
    errors: {
      load: '加载失败：{{message}}',
      createToken: '创建失败：{{message}}',
      revokeToken: '撤销失败：{{message}}',
      resetLocalToken: '重置失败：{{message}}',
  ...
  status: {
    ...
    auditTime: '时间',
    auditOutput: '输出摘要',
    auditLoadFailed: '加载审计记录失败',
    auditEmpty: '暂无工具调用记录',
    auditDecisionSkipped: '跳过',
    auditDecisionApproved: '已批准',
    auditDecisionDenied: '已拒绝',
  ...
  im: {
    ...
    manage: '管理',
    feishuQrTitle: '扫码安装飞书机器人',
    feishuQrRescan: '重新扫码安装',
    feishuQrHint: '使用飞书 App 扫描下方二维码，无需手动填写 App ID / Secret',
    feishuQrDomain: '版本',
    feishuQrScan: '生成二维码',
    feishuQrLoading: '正在获取二维码...',
    feishuQrInstruction: '请使用飞书 App 扫描二维码完成安装',
    feishuQrCountdown: '二维码有效期剩余 {{seconds}} 秒',
    feishuQrExpired: '二维码已过期，请重新获取',
    feishuQrSuccess: '扫码成功！App ID 与 App Secret 已自动填入',
  ...
  extensions: {
    title: '扩展',
    subtitle: '统一管理 builtin 工具、技能与 MCP 服务',
    loadFailed: '加载扩展失败',
    enabledSuccess: '已启用',
    disabledSuccess: '已禁用',
    toggleFailed: '操作失败',
    ...
    reloadFailed: '重载失败',
    viewDetail: '查看详情',
    builtinNoToggle: '内置工具不可禁用',
    searchPlaceholder: '按名称、ID 或描述搜索',
    empty: '没有匹配的扩展',
    totalSummary: '已展示 {{filtered}} / {{total}}',
    columnName: '名称',
    columnSource: '来源',
    columnHealth: '健康状态',
    columnCapabilities: '能力',
    columnEnabled: '启用',
    source: {
      builtin: '内置',
      skill: '技能',
