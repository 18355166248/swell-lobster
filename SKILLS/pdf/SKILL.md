...
5. In your reply, use the `url` field to render the file card: `[filename.pdf](output_files[].url)` — always use `output_files[].url` as-is; never hand-write or truncate the URL (it contains a required `?localPath=` parameter)
6. **For dynamically generated scripts**: use `script_content` parameter to provide the script source inline — the tool will create the file automatically before running it. **Script path MUST be inside `$DATA_SKILLS_DIR/tmp/` (e.g., `$DATA_SKILLS_DIR/tmp/fill_form.py`)**; writing to `SKILLS/` is not allowed.
