...
| 日记功能                                                      | ✅   | ✅   |
| 日记增强：心情记录 + 记忆联动                                 | ✅   | ✅   |
| 阶段 9：Embedding 配置 + 语义记忆检索 + `web_search` 基础能力 | 🟡   | 🟡   |

### 核心功能完整度

当前已完成阶段 1-7，阶段 9 已部分落地，系统具备：
...
- 会话导出（Markdown / JSON）
- 日记心情记录与记忆自动提取
- Embedding 配置、语义记忆检索降级链路、多提供商网络搜索
...
阶段9：向量记忆 + 网络搜索 🟡 → 语义检索、多提供商 web_search、迁移与配置收口待补
...
1. 主干稳定化：统一使用 `npm run verify` 作为仓库级健康检查入口。
2. 阶段 9 收口：补迁移脚本、配置优先级和剩余测试，不重做现有实现。
