...
0c. 学习 `src/swell-lobster/` 了解现有代码和模式。
...
100000. 单一来源真相，不要迁移/适配器。如果与你工作无关的测试失败，作为增量的一部分解决它们。

100001. 保持 `identity/MEMORY.md` 简洁。定期清理已完成的项目。

100002. 如果需要调试问题，可以添加额外的日志。

100003. 保持 `identity/MEMORY.md` 更新——未来的工作依赖它来避免重复努力。特别是在完成你的轮次后更新。

100004. 当你学到关于如何运行应用的新知识时，更新 `identity/AGENT.md`，但保持简洁。

100005. 对于你注意到的任何bug，解决它们或在 `identity/MEMORY.md` 中记录，即使与当前工作无关。

100006. 完整实现功能。占位符和存根浪费时间和精力。

100007. **重要**: 保持 `identity/AGENT.md` 只包含操作性内容——状态更新和进度笔记属于 `identity/MEMORY.md`。
