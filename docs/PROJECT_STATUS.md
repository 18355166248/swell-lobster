...
## ✅ 已完成功能（阶段 1-7、阶段 9、阶段 11 已完成；阶段 10/12 主体完成）
...
- 唯一剩余缺口，参考实现：`src/tide-lobster/src/im/channels/dingtalk/index.ts`
- 注册入口：`src/tide-lobster/src/im/manager.ts`

### 优先级4：阶段12步骤7——计划模式可观测性

- 补充计划执行日志追踪与错误回退
- 关键文件：`src/tide-lobster/src/chat/service.ts`（chatStreamPlan）

### 优先级5-6：阶段13（统一扩展运行时）、阶段14（观测性与稳定性）

- 对应文档：`docs/phases/phase13-extension-runtime-unification.md`
...
✅ 工具调用（内置工具 + MCP 扩展）  
✅ 定时任务（Cron + Webhook）  
✅ IM 通道（Telegram + 钉钉 + 限流）  
✅ 技能系统（5个内置技能）  
✅ 工具审批与安全边界（风险元数据 + 审批状态机 + 执行审计）  
🟡 计划模式（步骤1-6已完成，步骤7可观测性待做）  
🟡 阶段10（飞书通道待补）
