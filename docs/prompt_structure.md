...
0c. 学习 src/swell-lobster/ 了解现有代码
...
SwellLobster 是一个全能自进化AI助手，核心目标是成为一个真正对用户有帮助的助手。
...
**位置**: `src/swell-lobster/core/agent.py`

构建最终的系统提示词（**v2：编译管线 + 预算裁剪 + 显式分段**）。

当前实现的“最终 system prompt”主要由 `Prompt Builder` 负责组装，而不是把各段直接字符串拼接在 `Agent._build_system_prompt()` 里：

- **Prompt Builder**：`src/swell-lobster/prompt/builder.py::build_system_prompt()`
- **预算裁剪**：`src/swell-lobster/prompt/budget.py`
- **记忆检索**：`src/swell-lobster/prompt/retriever.py::retrieve_memory()`
...
**位置**: `src/swell-lobster/core/agent.py` 中的 `PROMPT_COMPILER_SYSTEM`
...
**位置**: `src/swell-lobster/memory/extractor.py`
...
**位置**: `src/swell-lobster/core/brain.py` - `plan()`
...
**位置**: `src/swell-lobster/core/brain.py` - `generate_code()`
...
**位置**: `src/swell-lobster/core/brain.py` - `analyze_error()`
...
**位置**: `src/swell-lobster/core/agent.py` - `BASE_TOOLS`
