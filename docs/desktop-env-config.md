...
### 方式一：在用户主目录放全局 `.env` 文件（推荐）

桌面端启动时会自动加载当前用户主目录下的全局 `.env` 文件，无需修改系统环境变量；配置不会随着应用重装、升级或安装路径变化而丢失。

**全局配置路径**

| 系统    | 路径                                    |
| ------- | --------------------------------------- |
| Windows | `C:\Users\{用户名}\.swell-lobster\.env` |
| macOS   | `~/.swell-lobster/.env`                 |

不确定哪个路径实际生效时，查看日志文件，日志里会打印出 `global env dir` 与 `global env file`。

**操作步骤**

1. 打开对应目录（若不存在，手动创建 `.swell-lobster` 目录）
2. 新建文件 `.env`
3. 写入所需配置，保存
...
> **注意**：macOS 从 Finder / Dock 启动的 GUI 应用不会读取 shell 的 `export` 配置，推荐使用方式一（用户主目录下的全局 `.env` 文件）。
